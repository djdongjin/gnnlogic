...
class InfoMaxTrainer:
    def __init__(self, model_config, encoder_model, decoder_model,
        ...
        self.encoder_model.text_encoder.max_entity_id = max_entity_id
        self.encoder_model.graph_encoder.max_entity_id = max_entity_id
        ...
        if self.model_config.embedding.entity_embedding_policy in ['random','fixed']:
            ...
            self.encoder_model.text_encoder.randomize_entity_embeddings(padding=True)
            self.encoder_model.graph_encoder.randomize_entity_embeddings(padding=False)
        else:
        ...
        self.infomax_criteria = nn.BCEWithLogitsLoss()
    ...
    def get_optimizers(self):
        ...
        model_params_encoder_text = self.encoder_model.text_encoder.get_model_params()
        model_params_encoder_graph = self.encoder_model.graph_encoder.get_model_params()
        model_params_decoder = self.decoder_model.get_model_params()
        model_params = model_params_encoder_text + model_params_decoder
    ...
    def batchLoss(self, batch: Batch, batch_neg: Batch=None, mode='train'):
        ...
        if self.model_config.embedding.entity_embedding_policy == 'fixed':
            ...
            self.encoder_model.text_encoder.randomize_entity_embeddings(fixed=True, padding=True)
            self.encoder_model.graph_encoder.randomize_entity_embeddings(fixed=True, padding=False)

        # run the encoder
        encoder_outputs, encoder_hidden, pair_logits = self.encoder_model([batch, batch_neg])
        ...
        infomax_loss = 0.0
        if batch_neg:
            infomax_loss = self.infomax_criteria(pair_logits, torch.cat([torch.ones(batch.batch_size), torch.zeros(batch_neg.batch_size)], dim=0).to(pair_logits.device))
        # generate prediction
        ...
        return decoder_outp, loss + infomax_loss, conf
